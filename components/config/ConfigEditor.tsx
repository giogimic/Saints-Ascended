...
import { gameUserSettingsSchema, type GameUserSettingsFormData } from '@/lib/validations';
...
export function ConfigEditor({ 
  ...
  const categories = useMemo(() => {
    const cats = Array.from(new Set(configFields.map(field => field.category)));
    return ['All', ...cats];
  }, []);
  ...
  const handleLoadDefaults = () => {
    configFields.forEach(field => {
      if (field.defaultValue !== undefined) {
        // @ts-expect-error - Complex form typing, setValue handles type conversion internally
        setValue(field.key as keyof GameUserSettingsFormData, field.defaultValue, { shouldDirty: true });
      }
    });
  ...
  const renderField = (field: ConfigField) => {
    ...
    const fieldProps = {
      ...
      id: field.key,
      disabled: readOnly,
      className: clsx('input input-bordered w-full bg-base-200 border-base-content/20 focus:border-primary', {
      ...
      case 'boolean':
        return (
          <div className="flex items-center">
            ...
            />
            <label htmlFor={field.key} className="ml-2 text-sm text-base-content">
              {field.label}
            </label>
          </div>
        );
        ...
        {filteredFields.length === 0 ? (
          ...
          <div className="space-y-6">
            {categories.slice(1).map(category => {
              ...
              return (
                <div key={category} className="card bg-base-100 border border-base-content/10 shadow-lg">
                  <div className="card-body">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      {categoryFields.map(field => (
                        <div key={field.key} className="space-y-2">
                          ...
                          {errors[field.key as keyof GameUserSettingsFormData] && (
                            <p className="text-xs text-error">
                              {errors[field.key as keyof GameUserSettingsFormData]?.message}
                            </p>
                          )}
                        </div>
                      ))}
