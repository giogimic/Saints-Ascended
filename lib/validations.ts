...
export const serverConfigSchema = z.object({
  ...
  executablePath: z
    ...
    .refine(
      (path) => path.endsWith(".exe") || path.endsWith("ShooterGameServer"),
      {
        message: "Must be a valid server executable",
      }
    ),

  configDirectory: z.string().min(1, "Config directory is required"),

  serverDirectory: z.string().min(1, "Server directory is required"),

  map: z.string().min(1, "Map selection is required"),
...
export const gameUserSettingsSchema = z
  .object({
    // Basic Settings
    ...
    // Launch Options
    PreventHibernation: z.boolean().optional(),
    ForceRespawnDinos: z.boolean().optional(),
...
// Game.ini validation
export const gameIniSettingsSchema = z
  .record(
    z.string(),
...
export const validatePortAvailability = (
  port: number,
...
export const validatePortRange = (
  mainPort: number,
  queryPort: number,
  rconPort: number
): boolean => {
