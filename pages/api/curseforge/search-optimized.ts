...
function getCategoryConfig(category: string): { query?: string; categoryId?: number } | null {
  const configs: Record<string, { query?: string; categoryId?: number }> = {
    "Popular": { query: "" },
    "Utility": { query: "utility quality of life tools automation" },
    "Maps": { categoryId: 17 },
    "Gameplay": { query: "gameplay mechanics survival features" },
    "Structures": { query: "structures building construction" },
    "Creatures": { query: "creatures dinos dinosaurs animals" },
    "Cosmetic": { query: "cosmetic skins visual appearance texture" },
